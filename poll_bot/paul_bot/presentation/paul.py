...
mp_client = Mixpanel('855e3b2b79541bafb386d7e8ba5af320', consumer=mp_consumer)
...
class Paul(InteractionBot):
    def __init__(self, *args, **kwargs):
        ...
        async def poll(
            ...
            properties = {
                ...
                "options": str(options),
                "allow_multiple_votes": str(allow_multiple_votes),
                "allowed_vote_viewers": str(allowed_vote_viewers),
                "allowed_editors": str(allowed_editors),
                "allowed_voters": str(allowed_voters),
                "message_id": str(message.id),
                "channel_id": str(message.channel.id)
            }
            print(properties)
            mp_client.track(str(inter.author.id), "pollCreated", properties)
    ...
    async def toggle_vote(self, option: Option, voter_id: int):
        ...
        properties = {"voter_id": str(voter_id),
                      "poll_question": str(option.poll.question),
                      "option": str(option.label),
                      "channel_id": str(option.poll.channel_id)
                      }
        mp_client.track(str(voter_id), "TestVote",properties)
